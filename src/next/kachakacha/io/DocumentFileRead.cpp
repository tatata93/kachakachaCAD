    ...
    const std::string& where)
{
    ...
    case FeatureType::CreateGuideSurface: {
        ...
        if (definition.Find("continuity") != nullptr) {
            if (const JsonArray* orders = loader.ArrayAt(definition, "continuity", where)) {
                for (const JsonValue& item : *orders) {
                    made.continuity.push_back(item.Type() == JsonType::Number
                            ? static_cast<int>(item.AsNumber())
                            : 0);
                }
            }
        }
        if (definition.Find("supportSurfaces") != nullptr) {
            made.supportSurfaces = ReadIdArray(loader, definition, "supportSurfaces", where);
        }
        feature.definition = std::move(made);
        break;
    }
    ...
    case FeatureType::CreateFabricationModel: {
        ...
        made.splitSolidFaces = loader.BoolOr(definition, "splitSolidFaces", false);
