...
[[nodiscard]] bool CaseFacePushPullGrowsTheSolid(V2MainWindow& window)
{
    ...
    window.RunCommand("part.extrude");   // 面の縁を取り出して、下見を出す
    if (!Explain("矢印が出る", viewport.ExtrudeHandleShown())) {
        return false;
    }
    // **下見の間は文書を変えない**(R1 B2)。縁のワイヤーはまだ増えない。
    if (!Explain((std::string("下見では線が増えない(帯は ")
                     + window.StatusText().toStdString() + ")").c_str(),
            CountOfKind(window, kachakacha::v2::domain::EntityKind::Wire)
                == wiresBefore)) {
        return false;
    }
    ...
    // 確定して初めて、面の縁が文書のワイヤーになる。
    if (!Explain((std::string("確定すると面の縁が線になる(")
