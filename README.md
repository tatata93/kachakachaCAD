...
上部の「作図面」で3D空間内の紙を選び、「正対」で真正面へ向けます。「直線」「矩形」「円」を選ぶと、3D画面を2点クリックするか、押したままドラッグして直接作図できます。位置は指定したmm間隔のグリッドと既存ワイヤーの端点へスナップし、作図中はU/V座標と長さ・幅高さ・半径を表示します。直線は続けて点を置け、`Enter`、右クリック、または`Esc`で区切れます。作成後も編集タブから数値を修正できます。
