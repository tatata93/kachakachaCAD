...
| D-43 | 作図 | 面作成 | 線から面(線を選ぶだけ) | 線 1〜24 本。端点のつながりから閉じた輪を全部(弦のある輪は除く。1 本の線に面は 2 枚まで)。平面 / 4 辺で平らでない → 四辺面 / それ以外 → 境界面。行き止まりの端どうしのずれを挙げ、Enter で直線の端を寄せてから作る(円弧は動かさない)。元の線は残す | `app/LoopGraph`(輪の数え方を境界面と共有)+ `app/LoopFaces`、画面 `V2LoopFacesTool`、命令 `surface.from_lines` | 帯の面作成の先頭「線から面」 | あり(輪は実線、ずれは赤系の破線と ×) | HP-LF-01〜03 | PC_TESTED(f526c00) | 2026-09-22 オーナー決定(a ずれは伝えて寄せるか断るか選ばせる b 元の線は残す c ボタンを足す)。core: loop_faces_tests(15)。PC f526c00: HP-LF-01〜03 通過(画面の目視はオーナー未確認) |
