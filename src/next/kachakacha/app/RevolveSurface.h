...
//! 回転面を作った。V2 でも同じにする: ここは **回した断面の並び** を作るだけで、
//! 面そのものは形状ガイド(断面のロフト)が作る。回転面の専用の面は持たない
//! (持つと、展開・型紙・部品の道が形状ガイドと二重になる)。
//!
...
[[nodiscard]] base::Result<RevolveRequest> MakeRevolveRequest(
    ...
    const std::vector<geometry::CurveSegment>& axis, double angleDeg, int sections);
