...
| AT-SRF-005 | 部分 | tests_v2/surface_edit_input_state_tests.cpp(縁2本を押して決め、同じ面で縁を替え、同じ縁は外す)+ tests_v2/kernel_surface_edit_tests.cpp(隣の縁を角で合わせ先の面に沿わせて張り直し G1 を測る)。PC 66d3cac で kernel 通過。人の操作の自己試験 HP-SE-03(隣り合う面の縁を押して G1 で合わせ、元の面は残り、取り消しで消える)を足した。残り: HP-SE-03 の PC 確認 |
