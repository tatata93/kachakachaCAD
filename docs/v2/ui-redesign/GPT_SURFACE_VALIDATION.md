...
全ゲートの証明ファイルは1210b82f5を記録している。再開時には他AIの9416a5916
（U/V格子）と統合6afe53b14が同ブランチに入り、通常exeも再ビルドされていた。
後続の追加撮影・GPT限定6/6はその既存exeで実行したもの。画像のU/V格子は他AIの追加分。
後続変更を保持し、全ゲートの検証対象を後続コミットまで広げて記載しない。
最終の記録だけは_GO.cmd --publish-docs-onlyで送る。
