...
| AT-UIX-008 | 済 | tests_v2/view_orientation_tests.cpp(26区画すべてで正対でき、視線と上向きが直交する。ドラッグ量と回転量が比例し、刻んでも一気でも同じ姿勢になる。89度まで回しても90度へ寄らない。離した瞬間も1時間後も変化0。感度0.25/0.05/1.0deg-px、クリック15度、相対軸は選択が無ければ断る。視点の操作板はV1同等で、軸ごとの色付きの輪3本と両端の矢じり6個、上下左右4個、画面のまま回す2個、家、選択に正対、絶対と相対の切り替えの計15個。輪は軸ごとに決まった傾きの楕円で、視点を変えても動かない(掴む場所が毎回同じになる)。矢じりは輪の長い方の両端にあり、当たり判定は88pxのキューブで28px以上。互いに逆を向く。画面の軸で上下左右と画面のまま回せて、逆へ回せば元へ戻る) + cad_next --self-test(キューブの連続回転・非吸着・クリック正対・回転矢印がカメラだけを回す。操作板の15個が画面に入って押せる。回す12個はどれも押すと15度だけ回る。相対の輪は選択が無ければ断り姿勢を変えない。家で既定の視点へ戻り、絶対と相対を行き来できる) |
