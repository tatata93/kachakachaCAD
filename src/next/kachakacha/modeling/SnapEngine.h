...
enum class SnapKind {
    ...
    Center,            //!< 円・円弧の中心
    Quadrant,          //!< 四半点
    GridOnCurve,       //!< 線とグリッドの線が交わる点(線上の格子)
...
//! §6.1 に無い平面へ投影は、仕様に順位が無いため暫定で 5 と 6 の間に置く(オーナー判断待ち。同上)。
//! 延長線は **グリッドより弱い**(2026-09-21 オーナー判断)。延長線は「線」の案内で、
//! グリッドは「点」だからである。強くしていたころ、グリッドの上を狙っているのに
//! 近くの線の延長と重なっただけで、格子から外れた点が黙って入っていた。
//! 線上の格子(線とグリッドの線が交わる点)は **最近点より強い**(2026-09-21 オーナー報告)。
//! 最近点はポインタ直下に付いて距離がほぼ 0 になるので、同じ順位ではグリッドの点が
//! 永遠に選ばれず、「線上のグリッドにスナップできない」になっていた。
//! 平面上の自由点は最後の受け皿、画面交差は選ばない。
