...
KACHA_V2_TEST(gpt_fabrication, double_curvature_is_split_and_limits_are_honest)
{
    ...
    Require(made.Value().maximumMm<=.3 && made.Value().rmsMm<=made.Value().maximumMm,"measured max and RMS");
...
KACHA_V2_TEST(gpt_fabrication, brep_non_rectangular_boundary_and_pattern)
{
    ...
    domain::CreateFabricationModelDefinition definition; definition.method=2;
