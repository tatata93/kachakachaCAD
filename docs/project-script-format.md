...
> **旧版の記録:** このテキスト形式は現行版専用。Wire-first V2は旧 `.kcd` を読み込まず、
> `docs/v2/architecture-and-data.md` のZIP+JSON `.kcd2` 形式へ置き換える。
...
1つの断面や外形ガイドが複数線分でできている場合は、次の共通形式を使う。

```text
surface_grouped KIND NAME GROUP_COUNT WIRE_COUNT WIRE... [WIRE_COUNT WIRE... ...]
```

`KIND` は `planar`、`ruled`、`loft`、`guided_loft` のいずれか。各 `WIRE_COUNT` と直後のワイヤー名群が、1つの論理的な輪郭・断面・外形ガイドになる。各群の直線・円弧・ベジェ・B-splineは端点から自動整列される。`ruled` は断面群2つ、`loft` は断面群3つ以上、`guided_loft` は外形群2つに続けて断面群1つ以上を記述する。

例では、2本の線分からなる断面を2つ結ぶ。

```text
surface_grouped ruled side_skin 2 2 section_a_1 section_a_2 2 section_b_1 section_b_2
```
...
断面は車体の前から後ろの順に並べる。各断面は1本でも、`surface_grouped` で指定した複数線分でもよい。3断面以上では各断面を通り、断面位置で長手方向の接線が連続する三次補間を使う。元ワイヤーを編集すると面を再計算する。

### 外形ガイド2本と任意断面からガイド付きロフト面

```text
surface_guided_loft NAME GUIDE_A GUIDE_B SECTION_1 [SECTION_2 ...]
```

115系前面上部のように、直交する作業平面上の外形を両側のガイドとして残しながら面を張る。最初の2群が外形ガイドで、以降は1群以上の断面。各群は1本でも複数線分でもよい。断面の両端は各ガイドへ0.05 mm以内で接続する必要がある。ガイドの向き、断面の向き、断面順は自動整列する。断面が端部までない場合は、ガイド端同士を結ぶ仮想断面を補う。
