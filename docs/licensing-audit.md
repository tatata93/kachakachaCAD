...
- リポジトリ直下に `LICENSE` がないため、kachakachaCAD本体を第三者が利用・改変・再配布できる条件は未決定である。一般公開前に必ず選ぶ。

## 実際に配布される部品

| 部品 | 使用版 | 配布形態 | 主な条件 |
|---|---:|---|---|
| kachakachaCAD | 0.1.0 | EXE | 本体ライセンス未選定 |
...
ソフト本体を今後も自由ソフトとして守る案。ソース公開と再配布を認める一方、改変版も同じ自由を保つ。Qt/OCCTとの説明がまとまりやすい。作成モデルには適用されない。非公開プラグインを将来認めたい場合は設計に注意が必要。
