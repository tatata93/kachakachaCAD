...
Qt版では、画面からプロジェクトの新規作成・読み込み・保存、作業平面の数値作成、3Dワイヤーと平面上ワイヤーの作成、選択物の数値編集、複数ワイヤーの直接移動・コピー・ミラー複製、直線のトリム・延長、交差直線のC面取りとR丸め、削除、Undo/Redoができます。保存時は一時ファイルを介して置き換えるため、書き込み途中の既存ファイルを残しにくい方式です。
...
編集タブの「移動」「コピー」は、選択したワイヤーの基準点と移動先を3D画面で順に指定します。「ミラー複製」は作図面上で軸の2点を指定します。いずれも確定前の形を破線で表示し、複数選択したワイヤーをまとめて処理します。2本の直線を選択して「交点まで」を押すと、短い場合は延長し、交差部分が余る場合はトリムして同じ交点へ合わせます。
