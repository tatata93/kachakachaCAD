...
| ID | 入口 | 操作 | 実装 | 受入試験 | 状態 |
|---|---|---|---|---|---|
| D-44 | 作図 → 面作成 → GPT版 / 形状メニュー | 外周＋内部線（近似）または複数断面、入力一覧/順序/反転、許容偏差・標本最大/RMS、面プレビュー → 確定 | GptSurface / OcctGptSurface / V2GptSurfaceTool、surface.gpt_create | AT-GPT-S001/002、HP-GPT-01〜03 | 実装・検証中、PC未検証 |
