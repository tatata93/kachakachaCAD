...
`minimumPartWidthMm` `manualBoundaries` `openingWires` `foldWires` `reliefCutWires`(切れ目、
無ければ空)`connectionWires`
...
2026-09-14 から部材ごとの曲げ半径 `bendRadiusMm`(mm の並び)と
`bendRadiusLock`(0 = 自動、1 = 固定。同じ長さの並び)も持つ。
自動の部材は 0 を書く。人が決めた値と、たまたま測れた値を、混ぜないためである。
**どの部材も固定していなければ、どちらの鍵も書かない。** 中身が同じ文書が
1バイト違う、ということにしないためである。無ければ全部自動として読む。
並びの長さが部材の数と合わなければ、合わない分は自動として読む。
