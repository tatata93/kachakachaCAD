...
| F-02 | 製作 | 近似 | 近似 | 作り方: 標準/少部品優先/精度優先/手動条件。**切る向き: 縦(上下に走る線で)/ 横(水平に走る線で)/ 自動 / U / V、縁の角で必ず割る、面 1 枚を何枚に** | 候補 A/B/C(実評価)、棚の欄。`fabrication::BandSplitDirection` + `ResolveSplitAxis`(縦横は世界の上下で軸を決める)、`CornerParameters`(縁の折れ)、`equalPartCount`(角の区間ごとに幅で配って実幅で等分) | 作り方カード → 候補の並べ方/欄の既定。既定 = 縦・角で割る・4 枚(オーナー指示 2026-09-25「ボタン一つで」) | あり(全部の面のレール) | HP-AP-01〜05 | PC で確かめる(HP-AP-05) | 作り方カード 4 枚(core ApproxPolicy / CandidateForPolicy)。候補を人が押せばそれが勝つ。HP-AP-03(PC c2a7b28: 自己試験 HP-AP の 4 本が通過)。2026-09-25: 面を 2 枚以上選ぶと面ごとに近似モデルを 1 つずつ(1 回で戻る)。band_approximation_tests +3、fabrication_options_tests、feature_definition_tests |
...
2026-09-25 D-44追加改修: PC_TESTED(`3102580a`: ctest182/182、V2自己試験355/355)。自動外周・番号・矢印・クリック修正をHP-GPT-05で確認。
