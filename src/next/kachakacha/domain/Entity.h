...
struct Entity {
    ...
    bool datum = false;
    //! 製作の「生成」(固定・展開)で作ったとき、どの近似モデルから作ったか(F-15)。
    //! 一覧で近似モデルの下の「生成物」に並べるための **由来** であって、依存ではない
    //! (近似モデルを消しても残り、近似モデルが変わっても作り直さない。固定したものは独立)。
    //! 近似モデルがもう無ければ、ふつうの種類の節に出る。
    std::optional<EntityId> generatedFrom;
};
