...
| ID | 表示名 | 入力/主要パラメータ | 出力 | 試験 |
| --- | --- | --- | --- | --- |
| `surface.gpt_create` | GPT版 | 独立した面生成。外周と内部線を既定で自動判別し、候補切替・番号/色/方向矢印・線クリックで手動修正。閉じた外周＋任意の内部線（近似）、または2つ以上の断面。入力一覧・順番・反転・標本偏差の最大/RMS付きプレビュー・確定・取消 | GuideSurface | AT-GPT-S001, AT-GPT-S002 |
...
| `part.split` | 分割(立体を平面で2つに) | 道具から始める。部品 1、平面 = いまの作業平面(棚の「ずらす」で法線の向きへ動かす) | 両側 2 つの Part(ShellSplit method=1、side=+1/-1。平面の点と法線を持つ。元の部品は隠す)。平面が部品を通らなければ KER-H005 | AT-FIN-003 |
