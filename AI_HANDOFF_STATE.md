...
REQUEST_ID: P1-EXTRUDE-R8 / Q1-Q5-R7 / DIAG-REFUSAL-R4
TASK_ID: Phase 1 押し出し / Q1〜Q5 / 断り方の作り
STATUS: HELD_FOR_HUMAN_DECISION
REVIEW_STATUS: 3回連続 BLOCKING で受付停止(AIR-E060)。**判定は書き換えない**
BASE: 54b5f34(3件とも同じ。R7 / R6 / R3 を見てもらったときの HEAD)
HEAD: 0acea2d(PC で build / ctest 141・自己試験 230 / 基盤自己試験 106 すべて PASS)
REVIEW_SCOPE:
  - P1-EXTRUDE-R8 … 押し出しの窓・棚・矢印・下見(9ファイル・227行)
  - Q1-Q5-R7 … 部材番号の読み取りと曲げ半径(5ファイル・127行)
  - DIAG-REFUSAL-R4 … 理由の読み口をそろえた分(2ファイル・94行)
  基盤(tools/ai-local/, docs/ai/)は**範囲に入れない**。
BUILD / TEST: PC 全部 PASS。雲: core 134/134、Qt 当て木 69 files
CREATED_AT: 2026-09-14
UPDATED_AT: 2026-09-15

### 製品側のレビュー履歴(判定は書き換えない)

| 区間 | 回 | 判定 | 直したか |
| --- | --- | --- | --- |
| P1-EXTRUDE | R5 / R6 / R7 | BLOCKING × 3 | 直した(下記) |
| Q1-Q5 | R4 / R5 / R6 | BLOCKING × 3 | 直した(下記) |
| DIAG-REFUSAL | R1 / R2 / R3 | BLOCKING × 3 | 直した(下記) |

R8 / R7 / R4 は enqueue されたが、**3回連続に達しているので受け付けられていない。**
中身は出来ていて、人がひと言くれれば、そのまま見てもらえる。

#### 押し出し(P1-EXTRUDE)で見つかったこと

- R5 … 棚は「面に垂直」と出しながら、矢印と下見は作業平面の法線へ進んでいた。
  輪郭が作業平面と同じ平面にある間は同じ向きになるので、既存の試験
  (両方 Z)では見つからなかった。作業平面だけを傾ける試験を足した。
- R6 B1 … 詳細の窓が、渡された決めごとを欄に映していなかった。開いて何も
  触らずに確定するだけで、向きが並びの先頭へ黙って変わっていた。
- R6 B2 … 向きの解き方が2通りしか無く、窓で選んだ X 方向などは矢印にも
  下見にも出なかった。確定のたびに棚を触る前の値へ戻されてもいた。
  → `ExtrudeDirectionForMode()` で7通りすべてを1か所で解く。
  **覚える形とカーネルへ渡す形を分ける**(`PreparedExtrudeChoice`)。
  あわせて棚が7通りの向きと5通りの終端を名前で出せるようにした。
- R7 B1 … 「詳細...」が窓の答えをその足で形にしていた。画面の矢印と下見は
  窓を開く前のままで、**見ながら決めることができなかった。**
  → 窓は棚・矢印・下見へ映して戻るだけにする。作るのは「確定」。
- R7 B2 … 向きを長さのまま矢印と下見に使っていた。`{0,0,7}` と距離 4mm で
  下見だけ 28mm 進んでいた。→ カーネルと同じ場所で単位にする。

#### 製作の近似(Q1-Q5)で見つかったこと

- R4 B1 … 見せた案が、どの模型のどの値に対する案かを持っていなかった。
- R4 B2 … 統合で捨てる値を、半径についてだけ前もって見せていた。
- R5 B1 … 見せた案と確定時に作り直した案を突き合わせていなかった。
- R5 B3 / R6 B1 … 読めない部材番号の門が `BendRadiusNow()` にしか無く、
  棚の実際の更新は空欄と同じ道を通って部材1の半径を出していた。
  → 欄を読むのは `ReadPartNumbers()` の1回だけ。
  **空欄と「書いてあるが読めない」を型で分ける。**

#### 断り方(DIAG-REFUSAL)で見つかったこと

- R2 B1 … `Diagnostics().front()` を26か所で直に読んでいた。空で並びの外を読む。
- R3 B1 … 理由が1つも無いとき、`FirstDiagnostic()` だけが代わりの理由を作り、
  `FirstCode()` と `FirstDetailsJa()` は空だった。そのため `FirstMessageJa()` が
  番号の無い一文になり、**断り方そのものが約束を破っていた。**
  → 代わりの理由は `MissingReason()` の1か所。

### パケットの日本語が化けていた(直した)

Codex への `request.md` と `diff.patch` は正しい UTF-8 だったが、印(BOM)が
無いので Windows PowerShell 5.1 は機械の文字コード(CP932)で読む。Codex は
`Get-Content` で読むので、**見どころも差分も意味の無い字列で届いていた。**
自己試験は自分で UTF-8 として読み直していたので気づけなかった。

→ パケットに BOM を付け、request.md にも読み方を書き、自己試験も
**レビューアーと同じ読み方**で確かめるようにした。直したあとのレビューは、
指摘が行番号まで正確になった。
...
## CODEX_REVIEW_REQUIRED: ずれの許容(2026-09-15 に答えを入れた。判定を待つ)

**症状(前の記録のまま)**

`OcctGuideSurface` の「作ったものを測り、通っていなければ捨てる」検査は、
許容を `max(modelLinearMm * 10, 1e-4)` = **0.0001mm** にしていた。
考え方は正しい。V1 は測らずに出していた。しかし値が数値誤差の桁である。
案内付きロフト(`MakePipeShell`)も曲線網(`MakeFilling`)も **作りからして近似**
なので、この値では **どんな形でも断られる**。HO の前頭部で 0.19mm 外れて断られ、
命令の一覧にあるのに使えない状態だった。「あるのに使えない」がいちばん困る。
しかも新しい「面を作る」の画面では、その2つが主要6方式のカードに出ている。

**入れた答え(3案のうち 2 と 3 を合わせたもの)**

許容は作り方で決まる、とした。決め方は core に置いた
(`src/next/kachakacha/modeling/SurfaceDeviationLimit.h`)。
**カーネルには数を書かない。**雲の側で試験できる場所へ出した。

| 作り方 | 通ることを約束できるか | 許容 |
| --- | --- | --- |
| 平面・ルールド・ロフト・回転体 | 通す | `max(modelLinearMm*10, 1e-4)`(これまでどおり) |
| 案内付きロフト・曲線網・境界埋め | 近づけるだけ | 0.25mm |

0.25mm の根拠は **後の工程が許している量** である。板材の曲げ近似
(`fabrication/BandApproximation.h` の `maximumDeviationMm`)が既定で
面から 0.25mm 外れることを許している。次の工程が 0.25mm 許しているのに
手前の面へ 0.0001mm を求めるのは、2500 倍ちぐはぐである。
数は1か所(`kFabricationDeviationMm`)に置き、由来を注釈に書いた。

**黙って通さない。**近づけて作った面は、通ったときも外れた量を必ず言う。

- カーネルが警告 `KER-S102` を返す(`docs/v2/diagnostic-catalog.md` に追加)
- 「面を作る」の「4. 状態」に
  「! 近づけて作る面です。指定した線から最大 0.190 mm 外れています。」と出る
- 数値誤差の桁(1e-9 など)は言わない。毎回出ると読まれなくなる

試験: `tests_v2/surface_deviation_limit_tests.cpp`(4件)。
通す作り方の許容が緩んでいないこと、実測 0.19mm が通り 0.30mm が通らないこと、
通ったときも量を言うこと、誤差の桁は言わないこと。

**見てほしいところ**

- 0.25mm を板材の曲げ近似から借りてよいか。別の基準にするなら何か。
- 「近づける/通す」の切り分け(`FidelityOf`)が方式ごとに正しいか。
  とくに `BoundaryFill` を近似側に入れたこと。
- 断らずに警告で通す範囲を、これ以上広げてよいか。
...
追記(2026-09-15)。ずれの許容を作り方ごとにした(上の項)。
案内付きロフトは「近づける作り方」なので、許容は 0.25mm になり、
通ったときも外れた量が `KER-S102` と「4. 状態」に出る。
`WithCorrection` を偽にしたことで断面が置いたとおりに使われ、
それでも外れるなら、外れた量が人に見える。**黙って違う形にはならない。**
この2件は同じところを触っているので、まとめて見てほしい。
