...
状態: 完了(2026-09-07、ブランチ `codex/unified-sheet-parts`。Windows全18試験、
Release自己診断、画面画像確認を通過)

内部の `Surface`(厚みのない形状)と `Plate`(板厚・材質・製作属性)は分離を維持する一方、
通常UIでは両方を一つの「面部品」として扱う。モデル一覧、選択表示、開口、厚み設定を
統合し、利用者が操作前に内部型を判別しなくても同じ作業を続けられるようにする。

- 選択した面または製作部品の状態を一か所で表示し、材料・板厚・厚み方向を直接設定する。
- 元面をコンボボックスから選び直す画面、面用/板材用に分かれた開口操作を廃止する。
- モデル一覧は面と板材を別カテゴリへ重複表示せず、「面部品」内で厚み未設定/材質・板厚を表示する。
- 厚みのない形状、同じ面から複数の製作仕様を作る場合、治具用の基準面は失わない。
- 実装意図・境界・後続作業を `docs/sheet-part-ui-spec.md` に記録し、別AIへ引き継げるようにする。

- なぜ: 現状は内部データ型がそのまま操作手順へ露出し、同じ位置の面と板材が別項目になり、
  操作ごとにどちらを選ぶか利用者へ判断させている。模型工作で必要なのは「どんな形で、
  何mmの何材をどう切るか」であり、型の違いを先に理解させることではない。
- 検証方法: 面のみ/面から板厚設定/板材の設定更新/面と板材それぞれへの開口/同一面の複数仕様/
  保存再読込を自動試験する。Windowsで `scripts\check.ps1` と `--self-test` を通し、
  画面スナップショットで一覧の重複と旧厚み化UIが消えたことを確認する。

### (9) Wire-first V2の仕様凍結と並行実装基盤

状態: 完了(2026-09-08、仕様commit `25d9982`。製品要件63件、構造要件12件、受入試験83件、
UIコマンド53件の対応漏れ0。Windows既存18試験とDebug実行ファイルの自己診断を通過)

利用者の主編集対象をワイヤーへ統一し、従来の「面/板材」を利用者向け概念から廃止するV2を、
別AIが設計判断を補わず実装できる粒度まで仕様化する。製品契約、データモデル、幾何契約、
UI操作、部材近似、保存形式、受入試験、作業パッケージを `docs/v2/` に固定する。

- 旧 `.kcd` の読込互換は持たない。
- V2は同じリポジトリ内に並行実装し、受入試験を通すまで現行版を削除しない。
- 実装担当は規範文書にない仕様を独断で追加せず、矛盾はADRで解決する。
- 完了条件は文書間の用語・契約・試験IDの整合確認と、実装順序・担当境界の確定。
