...
おまかせ(選んだ順の断面で一気に作る道)は同じ保存の形を使う。人の入口は「面を作る」に一本化したので、いまは自己試験だけが直接呼ぶ。

## 6b. 自由曲面と複数入力の試験(2026-09-22)

### AT-SRF-001 ロフト: 断面2〜任意 + ガイド0〜任意 + 中心線0〜1

2断面0本(ふつうのロフト)、3断面1本、3断面2本(従来の案内付きロフトの回帰)、3断面3本、5断面5本を作る。
ガイドが断面と交わらない・ガイドどうしで断面の順が逆転するものは理由付きで断る。3本目のガイドを動かすと面が変わる。

### AT-SRF-002 四辺面

4辺で作り、並びと向きが逆でも直して作る。閉じない4辺は離れ量を言って断る。内側の通る線0本/複数本。張り方(Coons/Stretch/Curved)が形に効く。

### AT-SRF-003 連続条件 G0/G1/G2

境界面・四辺面の辺ごとに G0/G1/G2 と支持面を指定し、作ったあとで折れ目と曲率の差を測る。支持面の無い G1/G2 は成功扱いしない。

### AT-SRF-004 曲線網(厳密 Gordon / 近似 Filling)

U2V2 と U5V4 を作る。交差不足・多重交差・交差順の矛盾を断る。片方しか成り立たない網は、成り立つ方を言う。

### AT-SRF-005 面を合わせる

直す面の縁を合わせ先の縁へ G0/G1/G2 で合わせ、元の面は残す。端が離れた縁は断る。滑らかさを測る。

### AT-SRF-006 面をつなぐ

2本の縁のあいだを、両端 G0/G1/G2 の面で渡す。張りの強さが形に効く。

### AT-SRF-007 面を整える

形を許容の内側に保ったまま制御点を減らす。前後の最大のずれと制御点の数を出す。許容に届かない・減らない・解析的な面は断る。

### AT-SRF-008 対称に写す

面を対称面に写し、対称面の上の縁で境目の折れ目を測る。対称面に接していなければ、境目が無いと言う。

### AT-SRF-009 U/V 線を取り出す

面の U/V 方向の線を、ふつうの線として取り出す。面の外(穴)を通るところは切る。

### AT-SRF-010 面へ投影(厳密)

線を作業平面の向きに面へ落とし、面の上の曲線(折れ線でない)にする。

### AT-SRF-011 面の解析

ゼブラ・平均曲率・ガウス曲率・U/V 線・曲率コーム・境目の連続・入力線からのずれを出す。可展性(ほぼ可展/二重曲率あり/強い二重曲率)を数値の基準とともに出し、平面・円筒・円錐で妥当。面が変われば更新する。

### AT-SRF-012 初心者向けの役割の分類と作り方の薦め

選んだ線を、つながり・交わり・閉じ方・向き・順番から決定的に分類し(断面・ガイド・境界・通る線)、作り方を理由と他の候補付きで薦める。役割は 3D で色分けし、線ごとに自動/断面/ガイド/境界/通る線を切り替えられる。
...
閉じた輪郭(矩形・穴つき)と回転軸の直線から、全回転・角度指定・対称回転の立体を作る。
体積が Pappus の予測(角度 × 面積 × 重心から軸までの距離)と合う。軸が輪郭の内側を通る・
軸が輪郭の平面に無い・角度が 0° 以下か 360° 超は理由つきで断る(SOL-003/004/005)。
保存して開き直しても同じ形に作り直せ、1 回の取り消しで消える。

### AT-SOL-002 ロフト立体

閉じた断面 2〜任意を、押した順に通して 1 つの立体を作る(平行な 2 断面なら体積は
角錐台の式と合う)。断面が 1 つ・開いた線・両端の断面が平面でない・隣の断面と重なる
ものは理由つきで断る(SOL-001/002/007)。ガイド付き・中心線付きは押せない形で理由を出す
(押せるのに何も起きないボタンを置かない)。

### AT-SOL-003 スイープ

閉じた輪郭を、1 本につながる経路(直線・円弧、何本でも)に沿って動かした立体を作る。
経路が輪郭の平面から始まらない・輪郭の面に沿う・つながっていない場合は理由つきで断る
(SOL-006/008)。まっすぐな経路の体積は 面積 × 長さ と合う。

### AT-FIN-001 立体の辺の丸め・面取り

箱の辺を 3D で押して選び(何本でも。押し直すと外れる)、半径 r で丸めると体積が
(1 - π/4) r² L だけ、距離 d で面取りすると d²/2 × L だけ減る。辺は真ん中の点で持ち、
保存して開き直しても同じ辺を丸め直す。1 回の取り消しで元の部品に戻る。無い辺・大きすぎる
半径は理由つきで断る(KER-R002 / KER-R001)。

### AT-FIN-002 部品のシェル

箱の上の面を 3D で押して抜く面に入れ(押し直すと外れる)、肉厚 t を残すと体積が
外 − (X−2t)(Y−2t)(Z−t) になる。面は面の上の点で持ち(辺のすぐそばを押しても面の内側へ寄せる)、
保存して開き直しても同じ面を抜き直す。1 回の取り消しで元の部品に戻る。無い面・厚すぎる肉厚は
理由つきで断る(KER-H002 / KER-H001・KER-H003)。

### AT-FIN-003 部品の分割

部品をいまの作業平面(法線の向きへずらせる)で分けると、両側の体積の和が元と同じで、両側が
2 つの部品になる。片側が離れた塊に分かれるときは数を知らせる。平面が部品を通らない(面と重なる
場合も)なら断る(KER-H005)。保存して開き直しても同じ平面・同じ側で作り直す。
...
順不同・向きばらばらの 40×20 の矩形で 800 mm²(厳密)、円 1 本で π r²、半円と直径で π r²/2 を検査する。
開いた線は UI-M002、平面に載らない輪は UI-M003 で断り、面積は寸法として残さない(UI-M004)。
画面では 1 辺を押しただけでも線の全体で測る。
...
- AT-GPT-S001: 閉じた外周（順不同・逆向き含む）と任意の内部線から近似面を作る。
  全入力を検査し、隙間・分岐・自己交差・不適合な役割・重複を拒否する。
  平面外周の内部線を持ち上げると面もふくらむ。外周外の線を黙って無視しない。
  全入力線の標本最大/RMS偏差を表示し、許容超過時は確定不可。
- AT-GPT-S002: 2つ以上の断面を一覧順に通す。開閉・辺数をそろえ、複合断面と順逆を扱う。
  両方式ともプレビュー/取消は文書不変、確定は1回でUndo/Redoでき、保存再読込で形状を再生成する。
  自動試験: gpt_surface_tests、HP-GPT-01〜04。PC実測はTASK_LEDGERへ記録。

### GPT版 自動外周・方向表示の回帰

AT-GPT-S001の追加: 外周4辺＋内部1辺をまとめて選び、外周/内部の事前指定なしで面を作る。
全入力を保持し、候補切替で外周が変わる。線と一覧の番号が対応し、線をクリックすると
行が選ばれ、方向反転が画面の矢印へ反映される。手動指定は再プレビューで上書きされず、
確定・取消で目印が残らない。HP-GPT-05で実ボタン・選択経路を通す。

### 製作近似GPT版
- AT-GPT-F001: 一方向曲面は1部材、二重曲率は少数部材、非矩形外周と穴を保持。最大/RMS、部材上限・最小幅を検査。
- AT-GPT-F002: 0/30/100%で輪郭実長維持、下見/取消は文書不変、確定Undo/Redoと保存再読込、型紙出力。
追加範囲はオーナー指示の製作近似(WP-09/製作UI統合)。HP-GPT-F01をWindows操作ゲートとする。
